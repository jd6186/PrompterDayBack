...
def find_user_by_user_id(user_id: int, db: Session):
    return db.query(User)\
        .filter(User.user_id == user_id, User.status_code == '1')\
        .first()
...
def find_user_by_nickname(nickname: str, db: Session):
    return db.query(User).filter(User.nickname == nickname, User.status_code == '1').first()
